...
export function renderCoreStats(state) {
  ...
  el.innerHTML = [
    ...
    statItem('Commerce Labor', formatNumber(world.commerceLaborAllocated ?? population.laborAssignedCommerce ?? 0)),
    statItem('Hemp Labor', `${formatNumber(world.hempLaborAllocated ?? 0)} / ${formatNumber(world.hempLaborRequired ?? 0)}`),
    statItem('Mulberry Labor', `${formatNumber(world.mulberryLaborAllocated ?? 0)} / ${formatNumber(world.mulberryLaborRequired ?? 0)}`),
    ...
    statItem('Farming Wage', formatDecimal(world.farmingWage ?? 0, 1)),
    statItem('Commerce Wage', formatDecimal(world.commerceWagePerWorker ?? 0, 1)),
    statItem('Institution Wage', formatDecimal(world.institutionWage ?? 0, 1)),
    statItem('Hemp Wage', formatDecimal(world.hempWage ?? 0, 1)),
    statItem('Mulberry Wage', formatDecimal(world.mulberryWage ?? 0, 1)),
    statItem('Average Wage', formatDecimal(world.averageWage ?? 0, 1)),
    statItem('Labor Flow', world.laborFlowSummary ?? 'N/A'),
